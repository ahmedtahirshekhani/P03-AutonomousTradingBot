...
const loginAuth = (email, password, role) => {
    return new Promise((resolve, reject) => {
        ...
        axios.post(`/api/${role}-login`, jsonReq, {
            headers: {
...
const registerInvestor = (ntn, inv_email, analyst_email, name, phone, address) => {
    const req = {
    ...
    const jsonReq = JSON.stringify(req);
    return new Promise((resolve, reject) => {
        axios.post('/api/register-investor', jsonReq,
        {
            headers: {
                'Content-Type': 'application/json'
            }
...
const logout = (email, role) => {
    ...
    return new Promise((resolve, reject) => {
        axios.post(`/api/${role}-logout`, jsonReq, {
            headers: {
                'Content-Type': 'application/json'
                }
                })
